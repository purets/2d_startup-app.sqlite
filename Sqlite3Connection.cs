...
#pragma warning restore CS0649
    ...
    public TimeSpan BusyTimeout {
        ...
        set {
            _busyTimeout = value;
            if(this.Handle != NullHandle) SQLite3.BusyTimeout(this.Handle, (int)_busyTimeout.TotalMilliseconds);
        }
    }
    ...
    public void CreateIndex<T>(Expression<Func<T, object>> property, bool unique = false)
    {
        ...
        MemberInfo propertyInfo = mx.Member as MemberInfo;
    ...
    /// </returns>
    public virtual TableQuery<T> Table<T>() where T : new()
    {
    ...
    public int Insert(object obj, string extra, Type objType)
    {
        ...
        if(map.PK != null && map.PK.IsAutoGuid) {
            ...
            if(prop != null) {
                ...
                var fi = objType.GetField(map.PK.PropertyName);

                if(fi != null && fi.GetValue(obj).Equals(Guid.Empty)) {
                    fi.SetValue(obj, Guid.NewGuid());
                }
            }
        }
...
public class TableMapping {
    ...
    public TableMapping(Type type, CreateFlags createFlags = CreateFlags.None)
    {
        ...
        this.TableName  = Name(type);

        MemberInfo[] props = this.MappedType.GetProperties(BindingFlags.Public |
            BindingFlags.Instance | BindingFlags.SetProperty);

        props = props.Concat(this.MappedType.GetFields(BindingFlags.Public |
        ...
        foreach(MemberInfo p in props) {
            ...
            bool saveField = p.GetCustomAttributes(typeof(SaveAttribute), true).Length > 0;

            if(((p is PropertyInfo pa && pa.CanWrite && !typeof(Component).IsAssignableFrom(pa.PropertyType)) ||
                p is FieldInfo pf && !typeof(Component).IsAssignableFrom(pf.FieldType) &&
                (typeof(ValueType).IsAssignableFrom(p.DeclaringType) || saveField)) && !ignore) {
                //if(!p.Name.Equals("notnull", StringComparison.OrdinalIgnoreCase)) {
                // UnityEngine.Debug.Log(p.GetNiceName());
        ...
        if(this.PK != null)
            this.GetByPrimaryKeySql = string.Format("select * from \"{0}\" where \"{1}\" = ?", this.TableName,
                this.PK.Name);
        else
    ...
    public class Column {
        ...
        public Column(MemberInfo prop, CreateFlags createFlags = CreateFlags.None)
        {
            ...
            if(prop is PropertyInfo p) {
                this.ColumnType = Nullable.GetUnderlyingType(p.PropertyType) ?? p.PropertyType;
            } else if(prop is FieldInfo f) {
                this.ColumnType = Nullable.GetUnderlyingType(f.FieldType) ?? f.FieldType;
            }

            this.Collation = Orm.Collation(prop);

            this.IsPK = Orm.IsPK(prop) || (createFlags & CreateFlags.ImplicitPK) == CreateFlags.ImplicitPK &&
            ...
            // UnityEngine.Debug.Log(this.IsAutoInc);
        ...
        {
            if(_prop is PropertyInfo prop) {
                prop.SetValue(obj, val, null);
            } else if(_prop is FieldInfo info) {
...
public static class Orm {
    ...
    public static bool IsPK(MemberInfo p)
    {
        ...
        return attrs.Length > 0 || p.Name.Equals("id", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public static bool IsAutoInc(MemberInfo p)
    {
        ...
        // if( attrs.Length > 0 || p.Name.Equals("id", StringComparison.OrdinalIgnoreCase) ) {
...
public class SQLiteCommand {
    ...
    public IEnumerable<T> ExecuteDeferredQuery<T>(TableMapping map)
    {
        ...
        try {
            ...
            while(SQLite3.Step(stmt) == SQLite3.Result.Row) {
                object obj = typeof(ScriptableObject).IsAssignableFrom(map.MappedType)
    ...
    public void Bind(object val)
    {
        ...
        Bind(null, val.GetType() == typeof(System.Type) ? _conn.GetMapping(val.GetType()).Name(val.GetType()) : val);
    }
    ...
    {
        if(value == null) return SQLite3.BindNull(stmt, index);
        if(value is int) return SQLite3.BindInt(stmt, index, (int)value);
        if(value is string) return SQLite3.BindText(stmt, index, (string)value, -1, NegativePointer);

        if(value is byte || value is ushort || value is sbyte || value is short)
            return SQLite3.BindInt(stmt, index, Convert.ToInt32(value));

        if(value is bool) return SQLite3.BindInt(stmt, index, (bool)value ? 1 : 0);
        if(value is uint || value is long) return SQLite3.BindInt64(stmt, index, Convert.ToInt64(value));

        if(value is float || value is double || value is decimal)
            return SQLite3.BindDouble(stmt, index, Convert.ToDouble(value));

        if(value is TimeSpan) return SQLite3.BindInt64(stmt, index, ((TimeSpan)value).Ticks);

        if(value is DateTime) {
            if(storeDateTimeAsTicks) return SQLite3.BindInt64(stmt, index, ((DateTime)value).Ticks);

            return SQLite3.BindText(stmt, index, ((DateTime)value).ToString("yyyy-MM-dd HH:mm:ss"), -1,
                NegativePointer);
        }

        if(value is DateTimeOffset) return SQLite3.BindInt64(stmt, index, ((DateTimeOffset)value).UtcTicks);

        {
            if(value.GetType().IsEnum) return SQLite3.BindInt(stmt, index, Convert.ToInt32(value));
        }

        if(value is byte[])
            return SQLite3.BindBlob(stmt, index, (byte[])value, ((byte[])value).Length, NegativePointer);

        if(value is Guid) return SQLite3.BindText(stmt, index, ((Guid)value).ToString(), 72, NegativePointer);

        //if(value is Vector3)
        return SQLite3.BindBlob(stmt, index, System.Text.Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(value)),
            System.Text.Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(value)).Length, NegativePointer);
    ...
    private object ReadCol(Sqlite3Statement stmt, int index, SQLite3.ColType type, Type clrType)
    {
        ...
        //if(clrType == typeof(Vector3)) {
        return JsonConvert.DeserializeObject(System.Text.Encoding.UTF8.GetString(SQLite3.ColumnByteArray(stmt, index)),
            clrType);
        //}
...
public class PreparedSqlLiteInsertCommand : IDisposable {
    ...
    public int ExecuteNonQuery(object[] source)
    {
        ...
        if(r == SQLite3.Result.Constraint && SQLite3.ExtendedErrCode(this.Connection.Handle) ==
            SQLite3.ExtendedResult.ConstraintNotNull) {
            SQLite3.Reset(this.Statement);
...
public class TableQuery<T> : BaseTableQuery, IEnumerable<T> where T : new() {
        ...
        if(orderExpr.NodeType == ExpressionType.Lambda) {
            ...
            if(mem != null && mem.Expression.NodeType == ExpressionType.Parameter) {
                ...
                q._orderBys.Add(new Ordering {
                    ColumnName = this.Table.FindColumnWithPropertyName(mem.Member.Name).Name, Ascending = asc
                });
    ...
    public TableQuery<TResult> Join<TInner, TKey, TResult>(TableQuery<TInner> inner,
        Expression<Func<T, TKey>> outerKeySelector, Expression<Func<TInner, TKey>> innerKeySelector,
        Expression<Func<T, TInner, TResult>> resultSelector) where TResult : class, new() where TInner : class, new()
    {
        TableQuery<TResult> q = new TableQuery<TResult>(this.Connection, this.Connection.GetMapping(typeof(TResult))) {
            _joinOuter            = this,
    ...
    public TableQuery<TResult> Select<TResult>(Expression<Func<T, TResult>> selector) where TResult : class, new()
    {
    ...
    private static object ConvertTo(object obj, Type t)
    {
        ...
        if(nut == null) return Convert.ChangeType(obj, t);
    ...
    public T FirstOrDefault()
    {
        ...
        //UnityEngine.Debug.Log(typeof(T).FullName);
        TableQuery<T> query = Take(1);
        var ret = query.ToList().FirstOrDefault();

        if(ret == null) {
            if(typeof(ScriptableObject).IsAssignableFrom(typeof(T))) {
                var method = typeof(ScriptableObject).GetMethods(BindingFlags.IgnoreCase | BindingFlags.Public |
                    BindingFlags.Static|BindingFlags.FlattenHierarchy).FirstOrDefault(mi => mi.Name == "CreateInstance" && !mi.IsGenericMethod && mi.GetParameters().Length > 0 && mi.GetParameters()[0].ParameterType == typeof(Type));
